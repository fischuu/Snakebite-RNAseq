...
if(!is.element("snakemake",ls())){
  projFolder <- "/scratch/project_2002561/MastitisChallenge/RNASeq-iSeqTestRun/"
  pipelineFolder <- "/scratch/project_2002561/pipeline/"
  pipelineConfig <- "/scratch/project_2002561/MastitisChallenge/RNASeq-iSeqTestRun/Pipeline-RNA-seq_config.yaml"
  refAnnot.file <- "/scratch/project_2002561/MastitisChallenge/RNASeq-iSeqTestRun/Reference/Bos_taurus.ARS-UCD1.2.100.gtf"
}
```
...
for(i in 1:length(tmp)){
  tmpPos <- grep(samples[i,1],names(tmp))
  names(tmp)[tmpPos] <- samples[i,1] 
}
