...
if(sum(colnames(rsem.transcripts[,-1])==paste0(sampleInfo[,1],"-TPM"))!=nrow(sampleInfo)) stop("Sample quantification order does not match the sample metainformation order!!! Check that!!!")
```
...
First we perform a hierarchical clustering, just using euclidean distances as distance measure
...
And the same for the quantile normalised values
...
for(i in 2:ncol(sampleInfo)){
  pairs(expr.PCA$x[,1:3], col=as.factor(sampleInfo[,i]), pch=20)
}
}
```
...
if(ncol(sampleInfo)>1){
for(i in 2:ncol(sampleInfo)){
  pairs(expr.PCA$x[,1:cols], col=as.numeric(as.factor(sampleInfo[,i])), pch=20)
}
}
```
